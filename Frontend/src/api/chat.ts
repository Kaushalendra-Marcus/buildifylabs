 ...
 * get live stage callbacks while waiting; the resolved value is the same
 * `PipelineOutput` as the unary endpoint, and quota 429s surface as the same
 * `ApiError` the composer already handles.
 */
...
export async function sendQuery(
  ...
  return readStream(response, onStage);
}
