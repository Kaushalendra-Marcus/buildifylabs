/**
 * MessageStream (F3 + F5) — the message-stream column region (specs/14 §3):
 * the ONLY place visuals render. Renders the message list from the chat store
 * — the four `specs/14` §4 message types (user / answer / clarification /
 * fallback) plus the F5 system notices (§5.6: window-exhausted inline notice,
 * lifetime-cap card, error), and the cold-start first-load state (§5.7).
 */
